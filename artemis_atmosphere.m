function atm = artemis_atmosphere(h)
% Atmospheric model for re-entry simulation.

%% MODELLING CHOICE %%

% 1-Exponential atmosphere with constant scale height and static
% temperature.

% 2-Hybrid model using table values above 54 km and exponential model
% below.
...
h = max(h, 0); % Set bounds to prevent non-physical values.
...
switch ATM_MODEL
    ...
    case 1
        ...
        % Set scale height and calculate temperature and density.
        H = 7100;                  
        T = H * g0 / Rair;     
        rho = rho0 * exp(-h / H); 
    ...
    case 2
        ...
        % Convert altitude to km as the upper-atmosphere table is in km.
        hk = h / 1000;
        ...
        % Section coefficients controlling scale-height and temperature 
        ...
        % first section which satifies both conditions, returning 1 index
        % and the first match.
